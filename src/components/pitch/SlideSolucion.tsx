...
export default function SlideSolucion() {
  return (
    <div className="w-full h-full flex flex-col bg-white relative overflow-hidden">
      ...
      {/* Header */}
      <motion.header
        className="pt-12 px-16 pb-6 flex items-center justify-between"
      ...
      >
        <div>
          ...
          <p className="text-[#9A9187] text-lg mt-1">
      ...
      >
        <div className="flex gap-6">
          {/* Before */}
          <div className="flex-1 bg-[#FAFAF8] border border-[#E8E0D5] p-6">
            <p className="text-[#9A9187] text-xs font-bold uppercase tracking-wider mb-4">
              Antes
            </p>
            <div className="space-y-3">
              {beforeItems.map((item, index) => (
                ...
                >
                  <span className="material-symbols-outlined text-[#9A9187] text-xl">
                    {item.icon}
                  </span>
                  <span className="text-[#9A9187] text-sm line-through">
                    {item.text}
                  </span>
                </motion.div>
              ))}
          ...
          <div className="flex-1 bg-[#0D0D0D] p-6">
            <p className="text-[#C4B8A8] text-xs font-bold uppercase tracking-wider mb-4">
              Después
            </p>
            <div className="space-y-3">
              {afterItems.map((item, index) => (
                ...
                >
                  <span className="material-symbols-outlined text-[#C4B8A8] text-xl">
                    {item.icon}
                  </span>
          ...
          {workflow.map((step, index) => (
            <motion.div
              ...
              className={`flex-1 p-5 ${
                index !== workflow.length - 1 ? 'border-r border-[#0D0D0D]' : ''
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className="w-8 h-8 bg-[#0D0D0D] text-white flex items-center justify-center text-sm font-bold">
                  {step.num}
                </span>
      ...
      >
        <div className="flex justify-center gap-6">
          {badges.map((badge, index) => (
            <motion.div
              key={badge.text}
              className="flex items-center gap-2 px-5 py-3 bg-[#F5F0E8]"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1 + index * 0.05 }}
            >
              <span className="material-symbols-outlined text-[#0D0D0D] text-lg">
                {badge.icon}
              </span>
              <span className="text-[#0D0D0D] text-sm font-bold">
                {badge.text}
